...
class Reinforce():
    def __init__(self, sess, optimizer, policy_network, max_layers, global_step,
        ...
        var_lists = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES)
        self.sess.run(tf.compat.v1.variables_initializer(var_lists))
    ...
    def create_variables(self):
        with tf.compat.v1.name_scope("model_inputs"):
            # raw state representation
            self.states = tf.compat.v1.placeholder(tf.float32, [None, self.max_layers*4], name="states")

        with tf.compat.v1.name_scope("predict_actions"):
        ...
        # regularization loss
        policy_network_variables = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope="policy_network")
        ...
        with tf.compat.v1.name_scope("compute_gradients"):
            ...
            with tf.compat.v1.variable_scope("policy_network", reuse=True):
                self.logprobs = self.policy_network(self.states, self.max_layers)
                print("self.logprobs", self.logprobs)

            # compute policy loss and regularization loss
            self.cross_entropy_loss = tf.nn.softmax_cross_entropy_with_logits(logits=self.logprobs[:, -1, :], labels=tf.stop_gradient(self.states))
            self.pg_loss            = tf.reduce_mean(input_tensor=self.cross_entropy_loss)
            self.reg_loss           = tf.reduce_sum(input_tensor=[tf.reduce_sum(input_tensor=tf.square(x)) for x in policy_network_variables]) # Regularization
